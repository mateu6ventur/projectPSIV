...
def main():
    # Calcula l'arrel del projecte de forma robusta (funciona en terminal i Colab)
    try:
        _project_root = Path(__file__).resolve().parent.parent
    except NameError:
        _project_root = Path("/content/projectPSIV")
    if not (_project_root / "train").exists():
        _project_root = Path("/content/projectPSIV")
    ...
    parser.add_argument(
        "--input_dir", type=Path, default=_project_root / "train",
        help="Carpeta arrel del dataset original (default: <projecte>/train)"
    )
    parser.add_argument(
        "--output_dir", type=Path, default=_project_root / "data_augmented",
        help="Carpeta de sortida (default: <projecte>/data_augmented)"
    )
    parser.add_argument(
